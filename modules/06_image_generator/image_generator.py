...
프롬프트:"""
    ...
    def _crop_to_aspect_ratio(self, image: Image.Image, aspect_ratio: str) -> Image.Image:
        ...
        ⚠️ 중요: 무조건 16:9 비율로 자르기
        
        Args:
            image: PIL Image 객체
            aspect_ratio: 목표 비율 (무시됨, 항상 16:9로 자름)
        
        Returns:
            16:9 비율로 자른 PIL Image 객체
        """
        # 무조건 16:9로 변환
        aspect_ratio = "16:9"
        ...
        target_ratio = 16/9  # 항상 16:9
        
        width, height = image.size
        current_ratio = width / height
        
        logger.info(f"이미지 자르기: 현재 {width}x{height} ({current_ratio:.2f}) → 16:9 비율 ({target_ratio:.2f})")
        
        # 이미지가 이미 16:9 비율이면 자르지 않음
        if abs(current_ratio - target_ratio) < 0.01:
            logger.info("이미지가 이미 16:9 비율입니다. 자르기 생략")
            return image
        ...
        final_ratio = cropped_image.size[0] / cropped_image.size[1]
        logger.info(f"✅ 이미지 16:9 자르기 완료: {cropped_image.size[0]}x{cropped_image.size[1]} (비율: {final_ratio:.2f})")
