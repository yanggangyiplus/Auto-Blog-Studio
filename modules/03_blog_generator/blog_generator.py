"""
블로그 생성기 - RAG 기반 HTML 블로그 생성 (Gemini 전용)
"""
...
class BlogGenerator:
    ...
    def _init_llm(self):
        ...
        if "gemini" in self.model_name.lower():
            ...
            raise ValueError(f"지원하지 않는 모델: {self.model_name}. Gemini 모델만 지원합니다.")
...
{html}
    ...
    def extract_image_placeholders(self, html: str) -> list:
        ...
        - ###IMG1###, ###IMG2###, ###IMG3### 형식의 마커 추출
        - <img src="PLACEHOLDER" alt="..."> 형식도 지원 (레거시)

        Args:
            html: HTML 문자열

        Returns:
            플레이스홀더 정보 리스트 [{"description": "설명", "index": 순서}, ...]
        """
        placeholders = []
        
        # 방법 1: ###IMG1###, ###IMG2### 형식의 마커 찾기 (현재 프롬프트 방식)
        marker_pattern = r'###IMG(\d+)###'
        marker_matches = re.finditer(marker_pattern, html)
        
        for match in marker_matches:
            img_num = int(match.group(1))
            # 마커 인덱스는 1부터 시작하지만, 리스트는 0부터 시작
            placeholders.append({
                "index": img_num - 1,
                "description": f"블로그 본문에 적합한 이미지 {img_num}",
                "marker": match.group(0)
            })
        
        # 방법 2: <img> 태그 방식 (레거시 지원)
        if not placeholders:
            img_pattern = r'<img[^>]*src="PLACEHOLDER"[^>]*alt="([^"]*)"[^>]*>'
            img_matches = re.finditer(img_pattern, html)
        ...
        logger.info(f"추출된 이미지 플레이스홀더: {len(placeholders)}개")
        return placeholders
