...
VECTORDB_DIR = CHROMA_DB_PATH  # Premium 대시보드 호환성
...
# API Keys (Gemini만 사용)
GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY")  # Google Gemini API (필수)
PIXABAY_API_KEY = os.getenv("PIXABAY_API_KEY")  # Pixabay 이미지 API (선택)
...
# 네이버 블로그 카테고리 설정 (순서: IT/기술 → 경제 → 정치)
...
# 컨텍스트 설정
MAX_CONTEXT_CHARS = int(os.getenv("MAX_CONTEXT_CHARS", "12000"))  # 컨텍스트 최대 문자 수 (대략 3000 토큰)
